    ...
    for (const doc of grammarDocuments) {
        ...
        for (const rule of grammar.rules) {
            ...
            if (elementAnnotation) {
                ...
                const element = {
                    kind: 'element',
                    label: elementAnnotation.args.label || rule.name, // Fallback to rule name? Validation should ensure label exists.
                ...
                if (endpointAnnotations.length === 2) {
                    ...
                    if (endpoints) { // Ensure endpoints were valid
                        const relation = {
                            kind: 'relation',
                            type: relationAnnotation.args.type || rule.name, // Fallback? Validation should ensure label exists.
...
logPrefix // Added parameter
    ...
    for (const ann of propertyAnnotations) {
        ...
        properties.push({
            name: name,
            type: type, // Store original type string
            baseType: baseType,
            isArray: isArray,
